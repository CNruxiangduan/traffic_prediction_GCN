...
def get_adjacent_matrix(distance_file: str, num_nodes: int, id_file: str = None, graph_type="connect",data_type=None) -> np.array:
    ...
    if data_type=='metr-la':
        ids_path = os.path.join('../dataset/PEMS/metr-la/','sensor_ids_la.txt')
...
class METRDataset(Dataset):
    ...
    def __getitem__(self, index):  # (x, y), index = [0, L1 - 1]
        ...
        data_x, data_y = METRDataset.slice_data(self.flow_data, self.history_length, index, self.train_mode)
        data_x = METRDataset.to_tensor(data_x)  # [N, H, D]
        data_y = METRDataset.to_tensor(data_y).unsqueeze(1)  # [N, 1, D]
        return {"graph": METRDataset.to_tensor(self.graph), "flow_x": data_x, "flow_y": data_y}
    ...
    def pre_process_data(data, norm_dim):
        ...
        """
        norm_base = METRDataset.normalize_base(data, norm_dim)  # find the normalize base
        norm_data = METRDataset.normalize_data(norm_base[0], norm_base[1], data)  # normalize data
...
def metrla_get_loader(ds_name="metr-la"):
    
    if ds_name == 'metr-la':
        num_nodes = 207
   

    train_data = METRDataset(data_path=["../dataset/PEMS/{}/distances_la.csv".format(ds_name), "../dataset/PEMS/{}/metr_la.h5".format(ds_name)], num_nodes=num_nodes,
                             divide_days=[45, 14],
    ...
    test_data = METRDataset(data_path=["../dataset/PEMS/{}/distances_la.csv".format(ds_name,ds_name), "../dataset/PEMS/{}/metr_la.h5".format(ds_name)], num_nodes=num_nodes,
                            divide_days=[45, 14],
                            time_interval=5, history_length=6,
...
if __name__ == "__main__":
    train_loader, test_loader=metrla_get_loader()
